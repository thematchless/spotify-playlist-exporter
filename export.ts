...
async function getTokenWithPKCE(): Promise<{
	access_token: string;
	...
	if (!CLIENT_ID)
		throw new Error("Please SPOTIFY_CLIENT_ID in your .env file.");
	...
	if (!tokenRes.ok)
		throw new Error(`Token-Error: ${tokenRes.status} ${await tokenRes.text()}`);
...
async function fetchAll<T>(url: string, accessToken: string): Promise<T[]> {
	...
	while (url) {
		...
		if (res.status === 429) {
			...
			await wait((retry + 1) * 1000);
			continue;
		}
		if (!res.ok) throw new Error(`HTTP ${res.status}: ${await res.text()}`);
		const data = (await res.json()) as SpotifyApiResponse<T>;
...
async function exportAll() {
	...
	for (const p of playlists) {
		...
		const out = {
			playlist: { id: p.id, name: p.name, total: p.tracks.total },
			tracks: simplified,
		};
		await writeFile(
			`export/${sanitize(p.name)}.json`,
		...
		exportIndex.push({
			id: p.id,
			name: p.name,
			file: `export/${sanitize(p.name)}.json`,
		});
	}
